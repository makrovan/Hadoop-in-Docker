...
xmlstarlet ed --inplace --update "/topology/gateway/provider[role='hostmap']/param[name='localhost']/value" --value 'hadoop-knox.docker.net' /usr/local/knox/conf/topologies/knoxsso.xml
...
xmlstarlet ed --inplace --update "/configuration/property[name='gateway.dispatch.whitelist']/value" --value '.*docker\.net.*;.*localhost.*' /usr/local/knox/conf/gateway-site.xml
