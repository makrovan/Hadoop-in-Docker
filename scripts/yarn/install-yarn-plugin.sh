...
xmlstarlet ed -L -s "//configuration" -t elem -n "property" -s "//property[last()]" -t elem -n "name" -v "xasecure.audit.jaas.Client.option.principal" -s "//property[last()]" -t elem -n "value" -v "rangeradmin/hadoop-ranger.docker.net@DOCKER.NET" /usr/local/hadoop/etc/hadoop/ranger-yarn-audit.xml
