...
mkdir -p $LOCAL_M2
set -x
docker run --rm --net=host -v "${src_folder}:/ranger" -w "/ranger" -v "${LOCAL_M2}:${remote_home}/.m2" $container_name $image_name $params
# docker run --rm --net=host -v /home/user/Projects/apache-ranger-2.4.0:/ranger -w /ranger -v /root/.m2:/.m2 --name ranger_build makrov/ranger_dev mvn -Pall -DskipTests=true clean compile package install